...
/// 
/// Modified by Michael Marchesan

// From: http://gregschlom.com/devlog/2014/06/29/Poisson-disc-sampling-Unity.html
...
namespace utils.random
	...
	public class PoissonDiscSampler
		...
		private readonly Polygon.AABB bb;
		private readonly float radius2;  // radius squared
		private readonly float cellSize;
		private Matrix<Point> grid;
		private List<Point> activeSamples = new List<Point>();
		...
		public PoissonDiscSampler(Polygon.AABB bb, float radius, ICollection<Point> predefined_points = null)
			{
			this.bb = bb;
			radius2 = radius * radius;
			cellSize = radius / Mathf.Sqrt(2);
			grid = new Matrix<Point>(Mathf.CeilToInt(bb.width / cellSize), Mathf.CeilToInt(bb.height / cellSize), new Point(0, 0));

			if (predefined_points != null) { foreach (var point in predefined_points) { AddSample(point); } }
			}

		/// Return a lazy sequence of samples. You typically want to call this in a foreach loop, like so:
		///   foreach (Point sample in sampler.Samples()) { ... }
		public IEnumerable<Point> Samples()
			{
			if (activeSamples.Count != 0) // There were predefined points // Modified by Michael Marchesan
				{
				foreach (var predefined_point in activeSamples) { yield return predefined_point; }
				}
			else // First sample is choosen randomly
				{ yield return AddSample(new Point(Random.value * bb.width, Random.value * bb.height)); }
			...
			while (activeSamples.Count > 0)
				...
				for (int j = 0; j < k; ++j)
					...
					Point candidate = sample + r * new Point(Mathf.Cos(angle), Mathf.Sin(angle));

					// Accept candidates if it's inside the rect and farther than 2 * radius to any existing sample.
					if (bb.contains(candidate) && IsFarEnough(candidate))
						{
						found = true;
		...
		private Point AddSample(Point sample)
			{
		...
		private struct GridPos
			...
			public GridPos(Point sample, float cellSize)
				{
